package com.iclick.spark.examples
...
object TestCombineByKey {
  def main(args: Array[String]): Unit = {
		...
		val conf = new SparkConf().setAppName("TestCombineByKeyJob").setMaster("local[1]").
      set("spark.sql.shuffle.partitions", "1").set("spark.network.timeout", "30s")
//		val sc = new SparkContext(new SparkConf().setAppName("TestCombineByKeyJob").setMaster("local[1]"))
